...
if mode == "validation":
    ...
    for fold in range(3):
        with wandb.init(project="Shopee", config=config, save_code=True, group=mode, job_type="Fold" + str(fold + 1),
            ...
            model = image_embedder(tfidf_dim=train_ds.text_vec.shape[1], embed_size=config["embed_size"], out_classes=train_ds.df.label_group.nunique())
...
if mode == "inference":
    with wandb.init(project="Shopee", config=config, save_code=True, group=mode, job_type=job_type,
        ...
        model = image_embedder(tfidf_dim=test_ds.text_vec.shape[1], embed_size=config["embed_size"], out_classes=train_ds.df.label_group.nunique())
